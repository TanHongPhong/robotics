...
// ================= Z =================
#define Z_PICK_STEPS 16000
#define BIN_Z_STEPS  14000
#define Z_START_US   1000
#define Z_RUN_US       50
#define Z_ACC_US        4
...
#define DEC_TIMEOUT_MS   10000  // Python không trả trong 10s -> auto SKIP
...
// ============================================================
#define SCAN_X_VMAX_MM_S       1000.0f
#define SCAN_Y_VMAX_MM_S       3000.0f
#define SCAN_MIN_MOVE_TIME_S      0.005f
#define SCAN_X_MIN_STEP_US  20U
#define SCAN_Y_MIN_STEP_US  10U

// ============================================================
// =================== RAPID MOVE =============================
// ============================================================
#define RAPID_X_MIN_STEP_US  50U
#define RAPID_Y_MIN_STEP_US  30U
...
const float PY_BASE[9] = {
  ...
  210 + DENTA_Y, 210 + DENTA_Y, 210 + DENTA_Y,
  420 + DENTA_Y, 420 + DENTA_Y, 420 + DENTA_Y
};
...
void moveXY_rapid(float x1, float y1){
  ...
  long ax = labs(dxStepsSigned);
  long ay = labs(dyStepsSigned);
  long m  = max(ax, ay);
  if(m <= 0){ curX=x1; curY=y1; return; }
  ...
  uint32_t baseTickUs = min((uint32_t)RAPID_X_MIN_STEP_US, (uint32_t)RAPID_Y_MIN_STEP_US);
  if(baseTickUs < 5) baseTickUs = 5;

  uint32_t lastX = micros();
  uint32_t lastY = micros();
  ...
  for(long i=0; i<m; i++){
    ...
    accX += ax;
    if(accX >= m && ax){
      while((uint32_t)(micros() - lastX) < RAPID_X_MIN_STEP_US) { }
      pulse(X_PUL);
      lastX = micros();
      accX -= m;
      xDone++;
    ...
    accY += ay;
    if(accY >= m && ay){
      while((uint32_t)(micros() - lastY) < RAPID_Y_MIN_STEP_US) { }
      pulse(Y_PUL);
      lastY = micros();
      accY -= m;
      yDone++;
    ...
    delayMicroseconds(baseTickUs);
  }

  while(xDone < ax){
    while((uint32_t)(micros() - lastX) < RAPID_X_MIN_STEP_US) {}
    pulse(X_PUL); lastX = micros(); xDone++;
  }
  while(yDone < ay){
    while((uint32_t)(micros() - lastY) < RAPID_Y_MIN_STEP_US) {}
    pulse(Y_PUL); lastY = micros(); yDone++;
  }
...
void moveZ_steps(long steps,bool down){
  digitalWrite(Z_DIR, down ? Z_DIR_FWD : Z_DIR_HOME);
  int d = Z_START_US;
  for(long i=0;i<steps;i++){
    if(stopReq) return;
    pulse(Z_PUL);
    if(d > Z_RUN_US) d -= Z_ACC_US;
    if(d < Z_RUN_US) d = Z_RUN_US;
    delayMicroseconds(d);
  }
}
...
void homeAll(){
  ...
  while(!hitZ()){
    if(stopReq) return;
    pulse(Z_PUL); delayMicroseconds(HOME_US);
  }
  ...
  while(!hitX()){
    if(stopReq) return;
    pulse(X_PUL); delayMicroseconds(HOME_US);
  }
  ...
  while(!hitY()){
    if(stopReq) return;
    pulse(Y_PUL); delayMicroseconds(HOME_US);
  }
...
enum RunModeState {
  ...
  // after decision:
  RM_DO_PICK,
  RM_AFTER_PICK_MOVE_NEXT,

  // mode2:
  RM_MODE2_SCAN_DONE_SENT,
  RM_MODE2_WAIT_LIST,
  RM_MODE2_PICK_GOTO,
  RM_MODE2_PICK_DO,
...
void startRun(){
  ...
  pendingDec = DEC_NONE;
  curP = 1;
  pickCount = 0; pickIdx = 0;

  if(mode == 1){
    Serial.println("[RUN] MODE 1 (event-based)");
    rm = RM_SCAN_MOVE;
  } else if(mode == 2){
    Serial.println("[RUN] MODE 2 (event-based)");
    rm = RM_SCAN_MOVE;
  } else if(mode == 3){
...
void runStateMachine(){
  ...
  switch(rm){
    ...
    case RM_SCAN_MOVE: {
      if(curP > 9){
        if(mode == 1){
          Serial.println("[RUN] MODE 1 DONE -> HOME");
          homeAll();
          rm = RM_IDLE;
        } else {
          // mode2 scan done
          Serial.println("SCAN_DONE");
          Serial.println("[MODE2] Waiting: LIST P..");
          rm = RM_MODE2_WAIT_LIST;
          tMark = millis();
        }
        break;
      }
      Serial.print("[SCAN] Move to P"); Serial.println(curP);
    ...
    case RM_SCAN_EVT_SENT: {
      ...
      // now wait decision only for mode1; for mode2 scan phase python will reply DEC SKIP anyway
      pendingDec = DEC_NONE;
      tMark = millis();
      rm = RM_WAIT_DECISION;
    } break;
    ...
    case RM_WAIT_DECISION: {
      ...
      // have decision
      if(pendingDec == DEC_PICK && mode == 1){
        Serial.print("[AI] DEC PICK at P"); Serial.println(curP);
        rm = RM_DO_PICK;
      }else{
        Serial.print("[AI] DEC SKIP at P"); Serial.println(curP);
        curP++;
        rm = RM_SCAN_MOVE;
      }
      pendingDec = DEC_NONE;
    ...
    case RM_DO_PICK: {
      pickFromHereToBin();
      if(stopReq) break;
      // after pick, go next quickly like your old code
      if(curP < 9){
        Serial.print("[RAPID] BIN -> P"); Serial.println(curP+1);
        moveXY_rapid(PX(curP+1), PY(curP+1));
      }
      curP++;
      tMark = millis();
      rm = RM_SCAN_SETTLE;
    ...
    // ===== MODE 2: WAIT LIST, PICK PHASE =====
    case RM_MODE2_WAIT_LIST: {
      // optional timeout 60s
      if((uint32_t)(millis() - tMark) > 60000UL){
        Serial.println("[MODE2] LIST timeout -> HOME");
        homeAll();
        rm = RM_IDLE;
        break;
      }
      // wait until Python sends LIST ... (handled in command parser)
    } break;

    case RM_MODE2_PICK_GOTO: {
      if(pickIdx >= pickCount){
        Serial.println("[MODE2] Picks done -> HOME");
        homeAll();
        rm = RM_IDLE;
        break;
      }
      ...
      moveXY_scan(PX(p), PY(p));
      tMark = millis();
      rm = RM_SCAN_SETTLE; // reuse settle
      // after settle we pick
      // BUT we need a distinct step to pick:
      // We'll use RM_MODE2_PICK_DO after settle
      // So mark by setting mode temporarily:
      // easiest: when settle done, if mode2 picking -> go to RM_MODE2_PICK_DO
    } break;
    ...
    case RM_MODE3_WAIT_GOTO: {
      // wait for commands: GOTO Pn / PICKNOW / DONE handled in command parser
    } break;
  ...
  // Special hook: after settle in mode2 pick phase go to pick-do
  if(rm == RM_SCAN_SETTLE && mode == 2 && pickCount > 0 && pickIdx < pickCount){
    // we are in mode2 picking, but RM_SCAN_SETTLE also used for scan phase
    // Distinguish by checking if scan phase finished: we only get here after RM_MODE2_PICK_GOTO.
    // We can set a flag using pickCount>0 and also ensure scan is done by rm came from RM_MODE2_PICK_GOTO.
    // Simple: if scan already done, rm won't be RM_SCAN_SETTLE unless we are picking.
    // Good enough in this architecture because scan phase transitions from RM_SCAN_MOVE directly.
  }
}
...
void parseListLine(const String& line){
  ...
  // start picking
  if(pickCount == 0){
    Serial.println("[MODE2] Empty list -> HOME");
    homeAll();
    rm = RM_IDLE;
  }else{
    rm = RM_MODE2_PICK_GOTO;
    // Immediately go first pick
    // but note: RM_MODE2_PICK_GOTO calls move, then settle; we need to switch to RM_MODE2_PICK_DO after settle.
  }
}
...
void handleCommand(const String& cmdRaw){
  ...
  // Decision from Python:
  // "DEC PICK" or "DEC SKIP"
  ...
  if(cmd.startsWith("LIST") || cmd.startsWith("list")){
    if(mode == 2 && rm == RM_MODE2_WAIT_LIST){
      parseListLine(cmd);
    }
    return;
  }

  // MODE3 manual commands
  if(mode == 3){
    ...
    if(cmd.equalsIgnoreCase("DONE")){
      Serial.println("[MODE3] DONE -> HOME");
      homeAll();
      rm = RM_IDLE;
      return;
    }
    if(cmd.startsWith("GOTO") || cmd.startsWith("goto")){
      for(int i=0;i<cmd.length();i++){
        if(cmd[i]=='P' || cmd[i]=='p'){
          char c = cmd[i+1];
          if(c>='1' && c<='9'){
            uint8_t p = (uint8_t)(c-'0');
            Serial.print("[MODE3] GOTO P"); Serial.println(p);
            moveXY_rapid(PX(p), PY(p));
            tMark = millis();
          }
        }
      }
...
void setup(){
  ...
  Serial.println("Cmd: H0 | OFFSET dx dy | MODE 1/2/3 | START");
  Serial.println("Python decisions: DEC PICK / DEC SKIP");
  Serial.println("Mode2: after SCAN_DONE, Python sends: LIST P2 P5 ...");
  Serial.println();
}
...
void loop(){
  ...
  // Fix: mode2 pick transition after settle
  // When mode2 picking, we used RM_SCAN_SETTLE; after settle we must do pick.
  if(mode == 2 && rm == RM_SCAN_SETTLE && pickCount > 0 && pickIdx < pickCount){
    if((uint32_t)(millis() - tMark) >= ARRIVE_SETTLE_MS){
      rm = RM_MODE2_PICK_DO;
    }
  }
